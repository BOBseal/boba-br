...
Post Deployment allow all endpoins for all instances to recieve bridges from
    setAllowedEndpoint(wormholeChainId , bridgeContractAddress);
    
    @bridgeContractAddress => bridge contract address at chain  @womrholeChainId
*/
...
// use wrap-unwrap version
contract BridgeV2 is TokenSender , TokenReceiver, Ownable{
    ...
    function quoteCrossChainDeposit(
        ...
        cost = deliveryCost + wormhole.messageFee() + fee;
    }
    ...
    //claim failed bridges with hash 
    function claimFailedBridge(bytes32 deliveryhash) public {
        ...
        amt = bridge.amount;
        tkn = bridge.token;
        require(amt > 0 && cl == false,"already claimed");
    ...
    // bridge native token revenue withdrawal -- if eth @eth = true && token = address(0) , else @eth = false
    function withdrawEthRevenue(uint256 amt, address to) public onlyOwner{
        require(feesTotal[address(0)]>= amt,"not enough amt");
        feesTotal[address(0)] = feesTotal[address(0)] - amt;
        payable(to).transfer(amt);
    }
    // bridge erc20 revenue withdrawal
    function withdrawTokenRevenue(address token , uint256 amt, address to) public onlyOwner{
        require(feesTotal[token]>= amt && token !=address(0),"not enough amt");
        feesTotal[token] = feesTotal[token] - amt;
        IERC20(token).transfer(to , amt);
    }
    // base native fee for bridges 
    ...
    function bridgeErc20(
        ...
        (fee,)= _processPartner(fee);
        bytes memory payload = abi.encode(recipient, uint8(1));
        ...
        feesTotal[address(0)] =feesTotal[address(0)] + fee;
    }
    ...
    function bridgeEther(
        ...
        ( fee,) = _processPartner(fee);
        bytes memory payload = abi.encode(recipient ,uint8(0));
        sendTokenWithPayloadToEvm(
            targetChain,
            targetBridge, // address (on targetChain) to send token and payload to
            payload, // data
            ...
            amount // amount 0 as native send as gas drop
        );
    ...
    // overrides and delivery Functions -- atleast 580_000 gas units for normal chains , 3mil for op-arb
    function receivePayloadAndTokens(
        ...
        (
            address recipient,
            uint8 type_
        ) = abi.decode(payload, (address, uint8));
        ...
        if(type_ == uint8(0) && tokenAmt > 0) { 
            uint256 a_ = tokenAmt * fMultiplier/1000;// fee
            uint256 f_ = tokenAmt - a_; // d amt 
            IWETH weth = tokenBridge.WETH();
        ...
        if(type_ == uint8(1) && receivedTokens[0].amount > 0){
            uint256 f_= tokenAmt * fMultiplier/ 1000;
            uint256 a = tokenAmt - f_;
            Bridges memory br = Bridges({
                fromChain :ch,
                sendType : type_,
                token: tokenAddress,
    ...
    function setProfitShareAddr(address addr) public {
        require(msg.sender == solutionsProviderAddress);
    ...
    function setProfitSharePercent(uint256 multiplier) public {
        require(multiplier < 20); // < 20 means 19
        require(msg.sender == solutionsProviderAddress);
    ...
    function _handleRef (address refered , address ref) internal {
        if(ref != address(0) && !referals[refered].refered && userData[refered].sendNonces <1){
